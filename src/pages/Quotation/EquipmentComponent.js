...
const EquipmentComponent = ({ EquipmentData, selectEquipment, setSelectEquipment }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [totalPrice, setTotalPrice] = useState(0);
  const [searchTerm, setSearchTerm] = useState('');

  useEffect(() => {
    const total = selectEquipment.reduce((acc, item) => acc + item.totalPrice, 0);
    setTotalPrice(total);
  }, [selectEquipment]);
  ...
  const handleEquipmentSelect = (equipment) => {
    const isAlreadySelected = selectEquipment.some((item) => item.id === equipment.id);
    if (isAlreadySelected) {
      setSelectEquipment(selectEquipment.filter((item) => item.id !== equipment.id));
    } else {
      setSelectEquipment([...selectEquipment, { ...equipment, quantity: 0, totalPrice: 0 }]);
    }
  };

  const handleQuantityChange = (id, quantity) => {
    setSelectEquipment(
      selectEquipment.map((item) =>
        item.id === id
          ? { ...item, quantity: parseFloat(quantity), totalPrice: parseFloat(quantity) * item.unitCost }
          : item
      )
    );
  };
  ...
  const isEquipmentSelected = (id) => selectEquipment.some((item) => item.id === id);
const relodedata=()=>{
  setSelectEquipment([]);
  setTotalPrice(0);
}
      ...
      <tbody>
        ...
        {selectEquipment.map((item, index) => (
          <tr key={item.id} className="hover:bg-gray-100 transition-colors">
            ...
            <td className="border p-1"> {`${item.name} - ${item.brand} - ${item.model}`}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
            <div className="bg-white p-8 rounded-lg shadow-lg w-[600px]">
              ...
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-semibold text-gray-700">Select Equipment</h3>
                <button onClick={handleCloseModal} className="text-gray-50 border border-red-400 px-3 py-1 rounded bg-red-400 hover:text-red-500 transition-colors">
              ...
              <ul className="space-y-4 overflow-auto h-70">
                {filteredEquipment.map((equipment) => (
                  <li key={equipment.id} className="flex items-center">
                    ...
                    />
                    <label className="text-gray-700 cursor-pointer">{`${equipment.name} - ${equipment.brand} - ${equipment.model}`}</label>
                  </li>
                ))}
              </ul>
              ...
              <div className="mt-6 flex justify-end space-x-4">
                ...
                <button
                  onClick={handleCloseModal} // Update this to handle modal actions if needed
