...
const ExclussionsComponent = ({ Exclussions,selectExclussions,setSelectExclussions }) => {
  const [isModalOpenFA, setisModalOpenFA] = useState(false);
  const [totalPrice, setTotalPrice] = useState(0);
  const [searchTerm, setSearchTerm] = useState('');

  useEffect(() => {
    const total = selectExclussions.reduce((acc, item) => acc + item.totalPrice, 0);
    setTotalPrice(total);
  }, [selectExclussions]);
  ...
  const handleEquipmentSelect = (equipment) => {
    const isAlreadySelected = selectExclussions.some((item) => item.id === equipment.id);
    if (isAlreadySelected) {
      setSelectExclussions(selectExclussions.filter((item) => item.id !== equipment.id));
    } else {
      setSelectExclussions([...selectExclussions, { ...equipment, quantity: 0, totalPrice: 0 }]);
    }
  };

  const handleQuantityChange = (id, quantity) => {
    setSelectExclussions(
      selectExclussions.map((item) =>
        item.id === id
  ...
  const filteredEquipment = Exclussions.filter((equipment) =>
    `${equipment.name} ${equipment.engineer} ${equipment.rate}`
      .toLowerCase()
      .includes(searchTerm.toLowerCase())
  );

  const isEquipmentSelected = (id) => selectExclussions.some((item) => item.id === id);
  const relodedata=()=>{
    setSelectExclussions([]);
    setTotalPrice(0);
  }
  return (
    <>
      <tbody>
        ...
        {selectExclussions.map((item, index) => (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
            <div className="bg-white p-8 rounded-lg shadow-lg w-[600px]">
              ...
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-semibold text-gray-700">Select Exclussions</h3>
              ...
              <ul className="space-y-4 overflow-auto h-70">
                {filteredEquipment.map((equipment) => (
                  <li key={equipment.id} className="flex items-center">
                    ...
                    />
                    <label className="text-gray-700 cursor-pointer">{`${equipment.name}`}</label>
                  </li>
                ))}
              </ul>
...
export default ExclussionsComponent;
