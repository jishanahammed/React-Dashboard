...
const InstallationComponent = ({ Installation,selectInstallation,setselectInstallation}) => {
  const [isModalOpenFA, setisModalOpenFA] = useState(false);
  const [totalPrice, setTotalPrice] = useState(0);
  const [searchTerm, setSearchTerm] = useState('');

  useEffect(() => {
    const total = selectInstallation.reduce((acc, item) => acc + item.totalPrice, 0);
    setTotalPrice(total);
  }, [selectInstallation]);
  ...
  const handleEquipmentSelect = (equipment) => {
    const isAlreadySelected = selectInstallation.some((item) => item.id === equipment.id);
    if (isAlreadySelected) {
      setselectInstallation(selectInstallation.filter((item) => item.id !== equipment.id));
    } else {
      setselectInstallation([...selectInstallation, { ...equipment, quantity: 0, totalPrice: 0 }]);
    }
  };

  const handleQuantityChange = (id, quantity) => {
    setselectInstallation(
      selectInstallation.map((item) =>
        item.id === id
  ...
  const filteredEquipment = Installation.filter((equipment) =>
    `${equipment.name}`
      .toLowerCase()
      .includes(searchTerm.toLowerCase())
  );

  const isEquipmentSelected = (id) => selectInstallation.some((item) => item.id === id);
  const relodedata=()=>{
    setselectInstallation([]);
    setTotalPrice(0);
  }
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
            <div className="bg-white p-8 rounded-lg shadow-lg w-[600px]">
              ...
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-semibold text-gray-700">Select Installation Services</h3>
