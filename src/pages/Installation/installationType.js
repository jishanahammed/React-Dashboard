...
const InstallationType = () => {
  const [Installation, setInstallation] = useState([
    { name: 'Installation', description: 'Installation for capturing solar energy.' },
  ]);

  const [newInstallation, setNewInstallation] = useState({ name: '', description: '' });
  ...
  const handleAddInstallation = () => {
    ...
    if (editingIndex !== null) {
      ...
      updatedInstallation[editingIndex] = newInstallation;
      setInstallation(updatedInstallation);
      setEditingIndex(null);
    } else {
      setInstallation([...Installation, newInstallation]);
    }

    setNewInstallation({ name: '', description: '' });
  ...
  const handleEditBrand = (index) => {
    ...
    setNewInstallation(Installation[index]);
    setIsModalOpen(true);
  };

  const handleDeleteBrand = (index) => {
    const updatedInstallation = Installation.filter((_, i) => i !== index);
    setInstallation(updatedInstallation);
  };
  ...
  return (
    <>
      <Breadcrumb
        pageName={"Installation Type List"}
        currentPage={"Installation Type List"}
      />
      <div className="grid grid-cols-1 shadow rounded-lg overflow-hidden bg-white dark:bg-gray-900 ">
        <div className="card-header bg-mygreen-80 dark:bg-gray-700 flex justify-between px-2 py-1 border-b border-gray-200">
          <h2 className="text-xl md:text-2xl font-bold text-gray-700 dark:text-gray-100">Installation Type</h2>
          <button
            onClick={openModal}
              ...
              <div className='border-b border-gray-300 p-3'>
                <div className='flex justify-between'>
                  <h3 className="text-lg font-bold text-mygreen-100">
                    {editingIndex !== null ? 'Update Installation' : 'Add Installation'}
                  </h3>
                  <button
                    onClick={closeModal}
              ...
              <div className="p-5">
                ...
                <div className="flex justify-center mt-5">
                  <button
                    ...
                    {editingIndex !== null ? 'Update Installation Type' : 'Add Installation Type'}
                  </button>
                </div>
              </div>
            </div>
